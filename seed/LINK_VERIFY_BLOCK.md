...
- Если web_extract вернул пустой или подозрительно короткий результат (страница отдаёт 200, но контент только на JS, геоблок, редирект на заглушку) — считай ссылку НЕ подтверждённой: дай рабочую альтернативу (официальный сайт бренда/магазина или точный поисковый запрос) и честно предупреди «страница может не открыться в браузере». Один curl-200 без реально прочитанного контента — не доказательство.
